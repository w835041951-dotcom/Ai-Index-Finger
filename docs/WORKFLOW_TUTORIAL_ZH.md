...
本教程适用于 `0.33.0-beta.12`。它从新建工作流开始，介绍每一种操作、变量与条件逻辑、循环、检查、调试和常见定位问题。

文中的手机截图来自 API 36 模拟器上的简体中文界面。不同设备的尺寸和系统设置页面可能略有区别。

> AI Index Finger 通过 Android 无障碍服务执行你主动创建和启动的工作流。界面信息和运行历史保存在本机；应用不申请 Android `INTERNET` 联网权限，也不向外传输工作流数据。请只自动化你有权操作的应用和数据。
...
截图选择使用的完整截图仅保留在内存中，不会上传。按截图点击会把你选中的裁剪区域作为模板保存在工作流中；导出该工作流或工作流库时，模板也会包含在导出文件内。

### 按截图点击的匹配与批量规则

仅在目标控件没有稳定无障碍选择器时使用**按截图点击**。该功能需要 Android 11 或更高版本，并且只会在所选目标应用当前可见的窗口内搜索。

1. 截取目标应用后，选择识别区域，并在区域内选择实际点击位置。
2. 能直接保存时会保留原始裁剪。最长边超过 1024 px 时会按比例缩小；PNG 超过 192 KiB 时先转为灰度，仍超限则每次按 85% 缩小，直到满足预算。只有最终模板会保存，点击位置会重映射到最终模板。
3. 默认使用**最佳匹配**：只点击一个最高分候选；同分时按从上到下、从左到右选择。默认最低相似度仍为 92%，默认尺寸仍为精确 1:1。
4. 只有确实需要批量操作时才开启**点击全部匹配**。候选来自同一张初始截图，按从上到下、从左到右点击；默认最多 20 次，硬上限 100 次，默认间隔 200 毫秒。

每次点击前都会重新检查目标包名、显示几何、目标窗口集合、模板脚印和手势坐标。若在已有成功点击后出现手势失败、窗口变化或超时，步骤会记录已完成数量，并且不会重试整个识图步骤；选择**继续下一步**时，运行历史会保留该 warning。
...
- **整体测试**：从编辑器测试当前工作流。
- **调试**：从首页启动后，悬浮调试器会显示在目标应用上方，并在第一个步骤前暂停。
- **停止**：立即请求停止当前执行。
- **运行历史**：查看结果、失败步骤、耗时和尝试次数；失败记录可以重新打开对应步骤。

悬浮调试器可以通过标题拖动，并自动吸附到最近的屏幕边缘。点击**下一步**会执行当前步骤，然后在下一个步骤前再次暂停；分支和重复中的嵌套步骤也会逐步暂停。暂停时点击**停止**不会执行当前步骤。

步骤执行期间，面板会暂时隐藏，避免挡住坐标手势或进入识图截图，并在下一个暂停点恢复到原来的位置。运行通知中的**下一步**和**停止**仍作为备用控制。如果 Android 无法创建悬浮调试器，本次调试不会启动；请重新连接自动化服务后重试。
